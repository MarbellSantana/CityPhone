...
import { Mail, MessageCircle, Minus, Plus, Search, ShoppingCart, Trash2 } from "lucide-react";
...
type InvoiceResult = { invoiceNumber?:string; cae?:string; caeDueDate?:string };
...
export default function VentasPage(){
 const[products,setProducts]=useState<Product[]>([]),[sales,setSales]=useState<Sale[]>([]),[cart,setCart]=useState<CartItem[]>([]),[search,setSearch]=useState("");
 ...
 useEffect(()=>{setProducts(load<Product[]>(KEYS.products,[]));setSales(load<Sale[]>(KEYS.sales,[]))},[]);
 ...
 const priceResults=useMemo(()=>{const q=priceSearch.toLowerCase().trim();return products.filter(p=>!q||p.name.toLowerCase().includes(q)||p.category.toLowerCase().includes(q)||(p.code||"").toLowerCase().includes(q)).slice(0,30)},[products,priceSearch]);
 ...
 function changeCartPrice(productId:number,value:string){const price=Math.max(0,Number(value||0));setCart(v=>v.map(i=>i.productId===productId?{...i,price}:i))}
 function changeProductPrice(productId:number,value:string){const price=Math.max(0,Number(value||0));const updated=products.map(p=>p.id===productId?{...p,price}:p);setProducts(updated);save(KEYS.products,updated);setCart(v=>v.map(i=>i.productId===productId?{...i,price}:i))}
 ...
 async function requestInvoice(sale:Sale){
   const r=await fetch(ARCA_ENDPOINT,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"invoice",sale,customer:{name:customer.trim()||"Consumidor final",phone,email,docType,docNumber,ivaConditionId}})});
   ...
   const nextSales=load<Sale[]>(KEYS.sales,[]).map(s=>s.id===sale.id?updated:s);
   save(KEYS.sales,nextSales);setSales(nextSales);
 ...
 async function confirmSale(){
   ...
   setCart([]);
   try{if(invoice){setNotice("Venta registrada. Solicitando CAE a ARCA...");await requestInvoice(sale)}else setNotice("Venta registrada sin facturar.")}
   catch(e){const message=e instanceof Error?e.message:"No se pudo conectar con ARCA";const failed=load<Sale[]>(KEYS.sales,[]).map(s=>s.id===sale.id?{...s,invoiceStatus:"Error" as const}:s);save(KEYS.sales,failed);setSales(failed);setNotice(`La venta quedó registrada, pero la factura no se emitió: ${message}`)}
 ...
 return <AppShell title="Ventas" subtitle="Registra ventas, revisa el historial y actualiza precios desde un mismo lugar." active="Ventas">
   <section className="workspace-grid sales-workspace">
     <div className="card workspace-card">
       ...
       <div style={{marginTop:18}}><h2>Carrito</h2>
         {cart.length===0?<div className="empty-state compact"><ShoppingCart size={30}/><b>No hay productos agregados</b></div>:<div className="sales-table">{cart.map(i=><div className="table-row" key={i.productId}>
           <div><b>{i.name}</b><small>Precio unitario editable</small></div>
           <span style={{display:"flex",gap:6,alignItems:"center"}}><button className="outline-action" onClick={()=>setCart(v=>v.map(x=>x.productId===i.productId?{...x,qty:Math.max(1,x.qty-1)}:x))}><Minus size={14}/></button><b>{i.qty}</b><button className="outline-action" onClick={()=>{const p=products.find(p=>p.id===i.productId);if(p)addProduct(p)}}><Plus size={14}/></button></span>
           <label style={{display:"flex",alignItems:"center",gap:5,fontSize:10,fontWeight:800}}>$<input aria-label={`Precio de ${i.name}`} type="number" min="0" step="1" value={i.price} onChange={e=>changeCartPrice(i.productId,e.target.value)} style={{width:92}}/></label>
           <strong>{money.format(i.price*i.qty)}</strong>
           <button className="ghost-button" onClick={()=>setCart(v=>v.filter(x=>x.productId!==i.productId))}><Trash2 size={15}/></button>
         </div>)}</div>}
     ...
     <div className="card workspace-card sticky-summary">
       ...
       {commission>0&&<div className="summary-line"><span>Comisiones + IVA</span><strong>-{money.format(commission)}</strong></div>}
       ...
       <div style={{marginTop:14,padding:14,border:"1px solid var(--border)",borderRadius:14,background:"var(--green-soft)"}}>
         ...
         {invoice&&<div className="form-grid" style={{marginTop:12}}><label className="field-label">Documento<select value={docType} onChange={e=>setDocType(e.target.value)}><option>DNI</option><option>CUIT</option></select></label><label className="field-label">Número de documento<input value={docNumber} onChange={e=>setDocNumber(e.target.value)} placeholder={subtotal>=10000000?"Obligatorio por el monto":"Opcional para consumidor final"}/></label><label className="field-label">WhatsApp del cliente <small>(opcional)</small><input autoComplete="off" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="Ej. 54911..."/></label><label className="field-label">Correo del cliente <small>(opcional)</small><input type="email" autoComplete="off" value={email} onChange={e=>setEmail(e.target.value)} placeholder="cliente@correo.com"/></label>{subtotal>=10000000&&!docNumber.replace(/\D/g,"")&&<small style={{gridColumn:"1 / -1",fontWeight:800}}>Para operaciones de $10.000.000 o más ARCA exige identificar al consumidor final.</small>}</div>}
       </div>
   ...
   <section className="card section-gap">
     <div className="card-heading"><div><h2>Ventas registradas</h2><p>Últimas 20 ventas guardadas en CityPhone.</p></div><span className="count-badge">{sales.length} ventas</span></div>
     {recentSales.length===0?<div className="empty-state compact"><ShoppingCart size={30}/><b>Todavía no hay ventas</b><span>Las ventas confirmadas aparecerán aquí automáticamente.</span></div>:<div style={{display:"grid",gap:9,marginTop:16}}>{recentSales.map(s=><article key={s.id} style={{display:"grid",gridTemplateColumns:"minmax(150px,1.4fr) minmax(130px,1fr) minmax(100px,.7fr) minmax(100px,.7fr)",gap:12,alignItems:"center",padding:"12px 14px",border:"1px solid var(--border)",borderRadius:13,background:"#fff"}}>
       <div><b style={{display:"block"}}>{s.customer||"Consumidor final"}</b><small style={{color:"var(--muted)",fontWeight:700}}>{new Date(s.createdAt).toLocaleDateString("es-AR")} · {new Date(s.createdAt).toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"})}</small></div>
       <div><small style={{display:"block",color:"var(--muted)",fontWeight:800}}>Productos</small><span style={{fontSize:11,fontWeight:800}}>{s.items.map(i=>`${i.qty}× ${i.name}`).join(" · ")}</span></div>
       <div><small style={{display:"block",color:"var(--muted)",fontWeight:800}}>Pago</small><b>{s.method}{s.secondaryMethod?` + ${s.secondaryMethod}`:""}</b><small style={{display:"block",marginTop:2,color:"var(--muted)"}}>{s.invoiceStatus||"Sin factura"}</small></div>
       <div style={{textAlign:"right"}}><small style={{display:"block",color:"var(--muted)",fontWeight:800}}>Total</small><strong style={{fontSize:16}}>{money.format(s.total)}</strong></div>
     </article>)}</div>}
   </section>

   <section className="card section-gap">
     <div className="card-heading"><div><h2>Modificar precios</h2><p>Cambia el precio de venta del producto. El nuevo valor queda guardado para las próximas ventas.</p></div></div>
     <div className="search-field" style={{marginTop:14}}><Search size={18}/><input value={priceSearch} onChange={e=>setPriceSearch(e.target.value)} placeholder="Buscar producto para cambiar precio..."/></div>
     <div style={{display:"grid",gap:8,marginTop:14}}>{priceResults.map(p=><div key={p.id} style={{display:"grid",gridTemplateColumns:"minmax(150px,1.5fr) minmax(90px,.5fr) minmax(130px,.7fr)",gap:12,alignItems:"center",padding:"11px 13px",border:"1px solid var(--border)",borderRadius:12}}>
       <div><b style={{display:"block"}}>{p.name}</b><small style={{color:"var(--muted)"}}>{p.category} · Stock {p.stock}</small></div>
       <div><small style={{display:"block",color:"var(--muted)",fontWeight:800}}>Costo</small><b>{money.format(p.cost)}</b></div>
       <label className="field-label" style={{margin:0}}>Precio de venta<input type="number" min="0" step="1" value={p.price} onChange={e=>changeProductPrice(p.id,e.target.value)}/></label>
     </div>)}</div>
   </section>
 </AppShell>;
}
