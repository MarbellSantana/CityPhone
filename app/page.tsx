...
const nav = [
  ...
  [Wrench, "Servicio técnico", "/servicio-tecnico"], [BarChart3, "Meses y reportes", "/"],
] as const;
    ...
    <div className="city-shell">
      <main className="main">
          ...
          <div className="header-actions">
            ...
            <div className="menu-wrap">
              ...
              {menuOpen && <>
                ...
                <nav className="dropdown-menu">{nav.map(([Icon, label, href], index) => <Link className={`nav-item ${index === 0 ? "active" : ""}`} href={href} key={label} onClick={() => setMenuOpen(false)}><Icon size={18}/><span>{label}</span></Link>)}</nav>
              </>}
            </div>
          </div>
        </header>
        ...
        <section className="welcome dashboard-intro">
          ...
          <Link href="/ventas" className="primary-button button-link"><Plus size={18}/> Nueva venta</Link>
        </section>
        ...
        <section className="dashboard-grid">
          <div className="card sales-chart-card">
            ...
            <div className="fake-chart" aria-label="Gráfico de ventas de los últimos siete días"><div className="chart-lines"><span/><span/><span/><span/></div><div className="bars"><i style={{height:"2%"}}/><i style={{height:"2%"}}/><i style={{height:"2%"}}/><i style={{height:"2%"}}/><i style={{height:"2%"}}/><i style={{height:"2%"}}/><i className="today-bar" style={{height:"2%"}}/></div><div className="chart-labels"><span>Lun</span><span>Mar</span><span>Mié</span><span>Jue</span><span>Vie</span><span>Sáb</span><span>Hoy</span></div></div>
          </div>

          <div className="card quick-card">
            <div className="card-heading"><div><h2>Accesos rápidos</h2><p>Lo que más usas en el local</p></div></div>
            <div className="quick-grid"><Link href="/ventas"><span><ShoppingBag size={19}/></span><b>Nueva venta</b><small>Registrar venta</small></Link><Link href="/inventario"><span><Box size={19}/></span><b>Inventario</b><small>Ver productos</small></Link><Link href="/servicio-tecnico"><span><Wrench size={19}/></span><b>Servicio</b><small>Nuevo equipo</small></Link><Link href="/caja"><span><Receipt size={19}/></span><b>Caja</b><small>Ver movimientos</small></Link></div>
          </div>
        </section>

        <section className="bottom-grid">
          <div className="card table-card"><div className="card-heading"><div><h2>Ventas recientes</h2><p>Últimas operaciones registradas</p></div><Link href="/ventas" className="text-button">Ver todas <ArrowUpRight size={15}/></Link></div><div className="sales-table">{recentSales.length === 0 ? <div className="empty-state compact">Todavía no hay ventas registradas.</div> : <><div className="table-head"><span>Venta</span><span>Producto</span><span>Pago</span><span>Total</span><span>Hora</span></div>{recentSales.map(s => <div className="table-row" key={s.id}><div><b>{s.id}</b><small>{s.customer}</small></div><span>{s.product}</span><span className="method">{s.method}</span><strong>{s.amount}</strong><small>{s.time}</small></div>)}</>}</div></div>
          <div className="card stock-card"><div className="card-heading"><div><h2>Stock bajo</h2><p>Requiere reposición</p></div><span className="warning-circle"><Activity size={17}/></span></div>{lowStock.length === 0 ? <div className="empty-state compact">No hay productos con stock bajo.</div> : <div className="stock-list">{lowStock.map(item => <div className="stock-item" key={item.name}><span className="product-mini"><Box size={17}/></span><div><b>{item.name}</b><small>Stock mínimo: 5</small></div><strong className="stock-number">{item.stock}</strong></div>)}</div>}<Link href="/inventario" className="outline-button button-link">Revisar inventario</Link></div>
        </section>
      </main>
    </div>
  );
